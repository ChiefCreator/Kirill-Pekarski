...
function animateOnScrollPortfolioSection() {
    ...
    const getHiddenWidth = (container) => {
        let localItems = container.querySelectorAll(".skill-card")
        return (localItems[localItems.length - 1].getBoundingClientRect().right - container.parentElement.getBoundingClientRect().right);
    }

    line2.style.transform = `translate(${-getHiddenWidth(line1)}px, 0)`;
    ...
    tl.to(line1, {x: () => -getHiddenWidth(line1), ease: "none"}, 0)
    ...
    let wrapperEnd = getOffsetTop(pageWrapper) + getHiddenWidth(line1);

    let wrapperTl = gsap.timeline({paused: true});
    wrapperTl.to(header, {opacity: 0, height: 0, duration: .5, ease: "linear"})
             .to(buttonStack, {transform: "translate(0, 0)", duration: .5, ease: "linear"}, "<")
