...
function animateCardPortfolioOnClick() {
    ...
    cards.forEach(card => {
        ...
        master.add(tl1, 0)
              ...
              .add(tl3, 1)
              .add(tl4, "<")
              .to(portfolioCard, {width: portfolioCardConfig.width, duration: 1, ease: "power4.in"}, "<")
        ...
        card.addEventListener("click", function() {
            ...
            let cardConfig = {
                left: parseFloat(card.getBoundingClientRect().left),
                top: parseFloat(card.getBoundingClientRect().top),
                width: parseFloat(card.getBoundingClientRect().width),
                height: parseFloat(card.getBoundingClientRect().height),
            }

            tl1.set(card, {opacity: 1})
               .set(portfolioCard, {left: cardConfig.left, top: cardConfig.top, width: cardConfig.width, height: cardConfig.height, zIndex: 4, opacity: 0})
               .to(portfolioCard, {left: portfolioCardConfig.left, top: portfolioCardConfig.top, height: portfolioCardConfig.height, opacity: 1, duration: 1, ease: "power4.out"})
